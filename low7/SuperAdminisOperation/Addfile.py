...
class Addfile:
    def __init__(self, win):
        ...
        self.informent.returnbut.clicked.connect(self.returnfun)
        self.informent.addfile.clicked.connect(self.select_fun1)
        self.informent.addmufile.clicked.connect(self.select_fun2)
    ...
    def select_fun1(self):
        self.path, _ = QFileDialog.getOpenFileName(self.informent, '请选择文件',
                                                   '/', 'ppt(*.ppt *.pptx);;word(*.docx *.doc)')
        if not self.path:
            QMessageBox.about(self.informent, "提示", '您没有选择任何文件!!')
            return
        self.sign = 1
        self.dow.fun2()
        self.dow.show()
    ...
    def fun1(self):
        try:
            ...
            QMessageBox.about(self.informent, "提示", '添加文件成功!!')
        except:
            QMessageBox.about(self.informent, "提示", '添加文件失败!!')

    def select_fun2(self):
        fname = QFileDialog.getExistingDirectory(self.informent, 'open file', '/')
        if fname:
            ...
            if self.files:
                ...
                QMessageBox.about(self.informent, "提示", '该目录没有任何文件!!')
        else:
            QMessageBox.about(self.informent, "提示", '您没有选择任何文件!!')
    ...
    def save_date(self, file, total, total2, filename1, filename2):
        ...
        if grade[:3] == "一年级":
            ...
            no = len(c.fetchall())
            c.execute("insert into First_Grade VALUES(?,?,?,?,?,?)",
            ...
            QMessageBox.about(self.informent, "抱歉", "六年级以上的此功能暂未实现！！")
        conn.commit()
        c.close()
        conn.close()
