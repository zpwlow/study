...
from PyQt5.QtWidgets import QMessageBox, QToolBox, QWidget
from PyQt5.QtGui import Qt, QBrush, QColor
import sqlite3
import SuperAdminisOperation
from SuperAdminisInterface import Controller_Logon_win, Function_win, User_Logon_win, Edit_Controller_win, Edit_user_win
from SuperAdminisInterface.Controller_news_win import CourseQlist
...
class Controller_news:
    ...
    def addusrfun(self):
        SuperAdminisOperation.win.splitter.widget(0).setParent(None)
        SuperAdminisOperation.win.splitter.insertWidget(0, User_Logon_win.User_Logon_win())

    def addcontrollerfun(self):
        SuperAdminisOperation.win.splitter.widget(0).setParent(None)
        SuperAdminisOperation.win.splitter.insertWidget(0, Controller_Logon_win.Controller_Logon_win())
    ...
    def edit_fun(self):
        ...
        for data in self.data:
            if data[0].isChecked():
                n += 1
        if n != 1:
            QMessageBox.about(self.news, "提示!", "抱歉，该功能只能选择一个用户进行编辑！！")
            return
        for data in self.data:
            if data[0].isChecked():
                if data[2] == "用户":
                    SuperAdminisOperation.win.splitter.widget(0).setParent(None)
                    SuperAdminisOperation.win.splitter.insertWidget(0, Edit_user_win.Edit_user_win(data[1]))
                elif data[2] == "管理员":
                    SuperAdminisOperation.win.splitter.widget(0).setParent(None)
                    SuperAdminisOperation.win.splitter.insertWidget(0, Edit_Controller_win.Edit_Controller_win(data[1]))
    ...
    def searchfun(self):
        ...
        if d == '':
            QMessageBox.about(self.news, "提示!", "搜索内容不能为空！！！")
            return
            ...
            if (b + c) > 0:
                ...
                QMessageBox.about(self.news, "提示!", "没有查到任何信息")
            ...
            if (b + c) > 0:
                ...
                QMessageBox.about(self.news, "提示!", "没有该用户的任何信息")
        ...
        self.news.win_layout2.addWidget(self.news.table, 2, 0, 18, 20)
    ...
    def deletedata(self):
        ...
        for data in self.data:
            if data[0].isChecked():
                n += 1
        if n == 0:
            QMessageBox.about(self.news, "提示!", "您没有选择任何用户，请您重新选择！！")
            return
        rely = QMessageBox.question(self.news, "提示!", "该操作会造成数据无法恢复！！！\n确定删除？？？", QMessageBox.Yes | QMessageBox.No,
                                    QMessageBox.Yes)
        if rely == 65536:
            return
        removeline = []
