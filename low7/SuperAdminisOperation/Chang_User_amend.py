...
from SuperAdminisInterface import Edit_user_win


class Chang_User_amend:
    def __init__(self, win):
        super(Chang_User_amend, self).__init__()
        self.chang = win
        sqlpath = './datas/database/Information.db'
        conn = sqlite3.connect(sqlpath)
        c = conn.cursor()
        c.execute("select * from User where number=(?)",
                  (self.chang.number,))
    ...
    def return_fun(self):
        ...
        SuperAdminisOperation.win.splitter.insertWidget(0, Edit_user_win.Edit_user_win(self.chang.number))
