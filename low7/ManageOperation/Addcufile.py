...
from ManageInterface import Course_news_win, Add_System_win
...
class Addcufile:
    ...
    def select_fun1(self):
        ...
        if end_file == '.ppt' or end_file == '.pptx':
            ...
            f.close()
            self.save_date(self.window.data, file, total, total2, '.zip', '.jpg')
            QMessageBox.about(self.window, "提示", '添加文件成功!!')
        else:
            QMessageBox.about(self.window, "提示", '添加文件失败!!')
    ...
    def select_fun2(self):
        ...
        if fname:
            ...
            if files:
                ...
                QMessageBox.about(self.window, "提示", '该目录没有任何文件!!')
        else:
            QMessageBox.about(self.window, "提示", '您没有选择任何文件!!')

    def select_fun3(self):
        ManageOperation.win.splitter.widget(0).setParent(None)
        ManageOperation.win.splitter.insertWidget(0, Add_System_win.Add_System_win(self.window.data))
